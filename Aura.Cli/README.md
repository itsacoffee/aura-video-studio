...
The Aura CLI Demo (`Aura.Cli`) is a cross-platform command-line tool that demonstrates the core functionality of Aura Video Studio. It runs on Linux, macOS, and Windows, providing a working proof-of-concept that validates all backend features without requiring the WinUI 3 UI.

## Purpose

1. **Cross-Platform Testing**: Validate backend logic on Linux CI runners
2. **Feature Demonstration**: Show all capabilities without Windows dependency
3. **Development Tool**: Quick testing of provider implementations
4. **Documentation**: Living example of the video generation pipeline
