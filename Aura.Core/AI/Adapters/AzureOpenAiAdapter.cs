...
public class AzureOpenAiAdapter : LlmProviderAdapter
{
    ...
    public override async Task<ProviderHealthResult> HealthCheckAsync(CancellationToken ct)
    {
        ...
        try
        {
            Logger.LogDebug("Performing health check for Azure OpenAI provider with model {Model}", _model);
            
            var modelExists = ModelRegistry.FindModel("Azure", _model) != null;
        ...
        {
            var elapsed = DateTime.UtcNow - startTime;
            Logger.LogError(ex, "Health check failed for Azure OpenAI provider");
