using System.Collections.Generic;

namespace Aura.Core.Errors;

/// <summary>
/// Provides documentation URLs and user-friendly help for error codes
/// </summary>
public static class ErrorDocumentation
{
    private static readonly Dictionary<string, ErrorDocumentationInfo> _errorDocs = new()
    {
        // Provider Errors (E100-E200)
        ["E100"] = new(
            "LLM Provider Error",
            "There was an issue with the language model provider (OpenAI, Anthropic, etc.)",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/troubleshooting/provider-errors.md#llm-errors"
        ),
        ["E100-401"] = new(
            "LLM Authentication Failed",
            "The API key for your language model provider is invalid or expired",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/troubleshooting/provider-errors.md#authentication"
        ),
        ["E100-429"] = new(
            "LLM Rate Limit Exceeded",
            "You've exceeded the rate limit for your language model provider",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/troubleshooting/provider-errors.md#rate-limits"
        ),
        ["E200"] = new(
            "TTS Provider Error",
            "There was an issue with the text-to-speech provider",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/troubleshooting/provider-errors.md#tts-errors"
        ),
        ["E200-401"] = new(
            "TTS Authentication Failed",
            "The API key for your text-to-speech provider is invalid or expired",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/troubleshooting/provider-errors.md#authentication"
        ),
        ["E400"] = new(
            "Visual Provider Error",
            "There was an issue with the image generation provider",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/troubleshooting/provider-errors.md#visual-errors"
        ),
        ["E400-401"] = new(
            "Visual Provider Authentication Failed",
            "The API key for your image generation provider is invalid or expired",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/troubleshooting/provider-errors.md#authentication"
        ),
        ["E500"] = new(
            "Rendering Error",
            "There was an issue rendering your video",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/troubleshooting/rendering-errors.md"
        ),
        
        // Validation Errors (E001-E003)
        ["E001"] = new(
            "Validation Error",
            "The input provided does not meet the required criteria",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/troubleshooting/validation-errors.md"
        ),
        ["E002"] = new(
            "Invalid Input",
            "One or more input parameters are invalid or missing",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/troubleshooting/validation-errors.md#invalid-input"
        ),
        ["E003"] = new(
            "Access Denied",
            "You don't have permission to perform this operation",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/troubleshooting/access-errors.md"
        ),
        
        // FFmpeg Errors (FFmpeg-specific)
        ["FFmpegNotFound"] = new(
            "FFmpeg Not Found",
            "FFmpeg is required for video rendering but was not found on your system",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/setup/dependencies.md#ffmpeg"
        ),
        ["FFmpegCorrupted"] = new(
            "FFmpeg Corrupted",
            "The FFmpeg installation appears to be corrupted or incomplete",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/troubleshooting/ffmpeg-errors.md#corrupted-installation"
        ),
        ["FFmpegFailed"] = new(
            "FFmpeg Processing Failed",
            "FFmpeg encountered an error while processing your video",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/troubleshooting/ffmpeg-errors.md#processing-errors"
        ),
        
        // Resource Errors
        ["OutOfDiskSpace"] = new(
            "Insufficient Disk Space",
            "There is not enough disk space to complete this operation",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/troubleshooting/resource-errors.md#disk-space-issues"
        ),
        ["OutputDirectoryNotWritable"] = new(
            "Output Directory Not Writable",
            "The specified output directory cannot be written to",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/troubleshooting/resource-errors.md#file-permission-errors"
        ),
        
        // API Key Errors
        ["MissingApiKey"] = new(
            "Missing API Key",
            "A required API key has not been configured",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/setup/api-keys.md"
        ),
        ["MissingApiKey:OPENAI_KEY"] = new(
            "Missing OpenAI API Key",
            "OpenAI API key is required for this operation",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/setup/api-keys.md#openai-api-key"
        ),
        ["MissingApiKey:ANTHROPIC_KEY"] = new(
            "Missing Anthropic API Key",
            "Anthropic API key is required for this operation",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/setup/api-keys.md#anthropic-api-key"
        ),
        ["MissingApiKey:ELEVENLABS_KEY"] = new(
            "Missing ElevenLabs API Key",
            "ElevenLabs API key is required for text-to-speech",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/setup/api-keys.md#elevenlabs-api-key"
        ),
        ["MissingApiKey:STABILITY_KEY"] = new(
            "Missing Stability AI API Key",
            "Stability AI API key is required for image generation",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/setup/api-keys.md#stability-ai-api-key"
        ),
        
        // Network Errors
        ["TransientNetworkFailure"] = new(
            "Network Connection Error",
            "Unable to connect to the remote service",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/troubleshooting/network-errors.md"
        ),
        
        // GPU/Hardware Errors
        ["RequiresNvidiaGPU"] = new(
            "NVIDIA GPU Required",
            "This operation requires an NVIDIA GPU with CUDA support",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/setup/system-requirements.md#gpu-requirements"
        ),
        
        // Platform Errors
        ["UnsupportedOS"] = new(
            "Unsupported Operating System",
            "This feature is not supported on your operating system",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/setup/system-requirements.md#operating-system-support"
        ),
        
        // Operation Errors
        ["E998"] = new(
            "Operation Cancelled",
            "The operation was cancelled by the user or system",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/troubleshooting/general-errors.md.md#operation-cancelled"
        ),
        ["E997"] = new(
            "Not Implemented",
            "This feature is not yet implemented",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/roadmap.md"
        ),
        ["E999"] = new(
            "Unexpected Error",
            "An unexpected error occurred",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/troubleshooting/general-errors.md"
        ),
        
        // Resilience/Circuit Breaker Errors
        ["CIRCUIT_OPEN"] = new(
            "Service Temporarily Unavailable",
            "The service is temporarily unavailable due to repeated failures. It will automatically recover.",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/troubleshooting/resilience.md#circuit-breaker"
        ),
        ["RETRY_EXHAUSTED"] = new(
            "Retry Attempts Exhausted",
            "All retry attempts have been exhausted for this operation",
            "https://github.com/Coffee285/aura-video-studio/blob/main/docs/troubleshooting/resilience.md#retry-policies"
        ),
    };

    /// <summary>
    /// Gets documentation information for a specific error code
    /// </summary>
    public static ErrorDocumentationInfo? GetDocumentation(string errorCode)
    {
        if (_errorDocs.TryGetValue(errorCode, out var doc))
        {
            return doc;
        }

        // Try to find a base error code (e.g., "E100" from "E100-429")
        var baseCode = errorCode.Split('-')[0];
        if (_errorDocs.TryGetValue(baseCode, out var baseDoc))
        {
            return baseDoc;
        }

        // Try to find a category code (e.g., "MissingApiKey" from "MissingApiKey:OPENAI_KEY")
        var categoryCode = errorCode.Split(':')[0];
        if (_errorDocs.TryGetValue(categoryCode, out var categoryDoc))
        {
            return categoryDoc;
        }

        return null;
    }

    /// <summary>
    /// Checks if documentation exists for an error code
    /// </summary>
    public static bool HasDocumentation(string errorCode)
    {
        return GetDocumentation(errorCode) != null;
    }

    /// <summary>
    /// Gets a fallback documentation URL for unknown errors
    /// </summary>
    public static string GetFallbackUrl()
    {
        return "https://github.com/Coffee285/aura-video-studio/blob/main/docs/troubleshooting/Troubleshooting.md";
    }
}

/// <summary>
/// Information about error documentation
/// </summary>
public record ErrorDocumentationInfo(
    string Title,
    string Description,
    string Url
);
