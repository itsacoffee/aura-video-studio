...
public class DependencyInstaller
{
        ...
        CancellationToken ct = default)
    {
        ...
        {
            var localAppData = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData);
            var installDir = Path.Combine(localAppData, "Aura", "piper");
            ...
            string downloadUrl;
            string fileName;
            string voiceModelUrl = "https://github.com/rhasspy/piper/releases/download/v1.2.0/voice-en-us-lessac-medium.tar.gz";
            ...
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            {
                if (RuntimeInformation.OSArchitecture == Architecture.X64)
                {
                    ...
                    fileName = "piper-windows.zip";
                }
                else
                {
                    throw new PlatformNotSupportedException("Piper TTS only supports x64 architecture on Windows");
                }
            }
            else if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
            {
                downloadUrl = "https://github.com/rhasspy/piper/releases/download/v1.2.0/piper_linux_x86_64.tar.gz";
                fileName = "piper-linux.tar.gz";
            }
            else if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
            {
                downloadUrl = "https://github.com/rhasspy/piper/releases/download/v1.2.0/piper_macos_x86_64.tar.gz";
                fileName = "piper-macos.tar.gz";
            }
            else
            {
                throw new PlatformNotSupportedException("Unsupported operating system");
            ...
            progress?.Report(new InstallProgress(5, "Downloading Piper TTS...", fileName, 0, 0));

            // Download Piper with retries
            var downloadPath = Path.Combine(Path.GetTempPath(), fileName);
            await DownloadWithRetriesAsync(downloadUrl, downloadPath, progress, ct).ConfigureAwait(false);

            progress?.Report(new InstallProgress(40, "Extracting Piper TTS...", "", 0, 0));
            ...
            progress?.Report(new InstallProgress(60, "Downloading voice model...", "en_US-lessac-medium", 0, 0));

            // Download voice model
            var voiceModelPath = Path.Combine(Path.GetTempPath(), "voice-model.tar.gz");
            await DownloadWithRetriesAsync(voiceModelUrl, voiceModelPath, null, ct).ConfigureAwait(false);

            progress?.Report(new InstallProgress(80, "Extracting voice model...", "", 0, 0));

            // Extract voice model
            await ExtractTarGzAsync(voiceModelPath, installDir, ct).ConfigureAwait(false);
            ...
            {
                File.Delete(downloadPath);
                File.Delete(voiceModelPath);
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "Failed to clean up download files");
            }

            _logger.LogInformation("Piper TTS installation completed successfully");
            return true;
        }
        catch (Exception ex)
        {
