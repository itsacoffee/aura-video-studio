...
export const Tooltip: React.FC<TooltipProps> = ({
  ...
  const adjustPosition = () => {
    ...
    if (position === 'top' && triggerRect.top < tooltipRect.height + 10) {
      ...
      position === 'bottom' &&
      viewportHeight - triggerRect.bottom < tooltipRect.height + 10
    ) {
      newPosition = 'top';
  ...
  const getPositionClasses = () => {
    const baseClasses =
      'absolute z-50 px-3 py-2 text-sm text-white bg-gray-900 dark:bg-gray-700 rounded-lg shadow-lg pointer-events-none';
  ...
  return (
    <div className="relative inline-block">
      ...
      {isVisible && (
        <div ref={tooltipRef} className={getPositionClasses()} style={{ maxWidth }} role="tooltip">
          <div className={getArrowClasses()} />
          <div className="relative z-10 whitespace-normal">{content}</div>
        </div>
      )}
    </div>
  );
...
export const HelpText: React.FC<HelpTextProps> = ({ text, position = 'top' }) => {
  return <Tooltip content={text} position={position} showIcon />;
};
