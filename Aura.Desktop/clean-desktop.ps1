...
$frontendPaths = @(
    ...
    # Note: OpenCut build artifacts are preserved below for development purposes.
    # OpenCut now runs natively in Aura.Web, so no server build is needed,
    # but developers may still want to test or modify the OpenCut source code.
)

# OpenCut workspace artifacts (for development - not bundled into desktop app)
$openCutPaths = @(
    "$projectRoot\OpenCut\node_modules",
...
$electronPaths = @(
    ...
    # Build-time resources (generated by build-desktop.ps1)
    "$projectRoot\Aura.Desktop\resources\backend"
    # Note: resources\opencut is no longer bundled - OpenCut runs natively in Aura.Web
)
