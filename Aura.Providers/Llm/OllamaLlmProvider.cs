...
public class OllamaLlmProvider : ILlmProvider
{
    ...
    private static HttpClient EnsureProperTimeout(
        HttpClient httpClient,
        int timeoutSeconds,
        ILogger logger)
    {
        var requiredTimeout = TimeSpan.FromSeconds(timeoutSeconds + 300);
        ...
        CircuitBreaker? circuitBreaker = null)
    {
        _logger = logger;
        _circuitBreaker = circuitBreaker;
        _httpClient = EnsureProperTimeout(
...
Return ONLY the transition text, no explanations or additional commentary:";
    ...
    /// Uses shared LlmScriptCleanup utility for consistency across all LLM providers
    /// </summary>
    ...
    {
        return LlmScriptCleanup.IsLlmMetaCommentary(line);
    }
