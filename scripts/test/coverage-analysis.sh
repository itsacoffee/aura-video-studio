...
|-----------|---------------|-----------------|--------|
| Backend (.NET) | ${LINE_COVERAGE:-N/A}% | ${BRANCH_COVERAGE:-N/A}% | $([ -n "$LINE_COVERAGE" ] && (($(echo "$LINE_COVERAGE >= $COVERAGE_THRESHOLD" | bc -l))) && echo "✅" || echo "⚠️") |
| Frontend (React) | ${LINE_COVERAGE:-N/A}% | ${BRANCH_COVERAGE:-N/A}% | $([ -n "$LINE_COVERAGE" ] && (($(echo "$LINE_COVERAGE >= $COVERAGE_THRESHOLD" | bc -l))) && echo "✅" || echo "⚠️") |
